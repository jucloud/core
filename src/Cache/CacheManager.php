...
class CacheManager extends Manager implements FactoryContracts
{
    ...
    {
        $config = Arr::get($this->container['app']['config']['cache'], 'stores');
        $config = $config[$name];
    ...
    {
        return Arr::get($this->container['app']['config']['cache'], 'default');
    }
    ...
    {
        return $config['prefix'] ?? $this->container['app']['config']['cache.prefix'];
    }
}
